...
export const StartSecondInningModal = ({ onClose, onUndo }) => {
  ...
  // In inning 2: the teams SWAP — old bowling team now bats.
  const inning1 = currentMatchData?.innings?.[0];
  const newBattingTeamId = inning1?.bowlingTeamId; // was bowling, now bats
  const newBowlingTeamId = inning1?.battingTeamId; // was batting, now bowls
  ...
  const getFilteredPlayers = (players, search, excludeIds = []) =>
    players.filter(
      (p) =>
  ...
  const strikerExclude = [selectedNonStriker?.playerId].filter(Boolean);
  const nonStrikerExclude = [selectedStriker?.playerId].filter(Boolean);

  const filteredStrikers = getFilteredPlayers(battingPlayers, strikerSearch, strikerExclude);
  const filteredNonStrikers = getFilteredPlayers(battingPlayers, nonStrikerSearch, nonStrikerExclude);
  const filteredBowlers = getFilteredPlayers(bowlingPlayers, bowlerSearch);

  const canStart = selectedStriker && selectedNonStriker && selectedBowler;
  ...
  const PlayerSearchSection = ({
    ...
    <div className="mb-4">
      ...
      {selected ? (
        ...
        <>
          <label className="input input-bordered flex h-10 items-center gap-2 rounded-xl">
          ...
          {search && (
            <ul className="mt-1 rounded-xl border border-base-content/15 bg-base-100 shadow-sm max-h-36 overflow-y-auto">
              {filteredList.map((player) => (
                <li
                  key={player.playerId}
                  onClick={() => { setSelected(player); setSearch(""); }}
              ...
              {filteredList.length === 0 && (
                <li
                  onClick={() => { const p = createPlayer(search); setSelected(p); setSearch(""); }}
    ...
    <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="relative flex w-full max-w-md flex-col overflow-hidden rounded-3xl bg-base-100 shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-base-200 px-5 py-4">
          <div>
            <h2 className="text-lg font-semibold">Start 2nd Inning</h2>
          ...
          {inning1 && (
            <div className="mb-4 rounded-xl bg-warning/15 border border-warning/30 px-4 py-3 text-center">
              <p className="text-sm font-semibold text-warning">
                Target: {inning1.runs + 1} runs in {currentMatchData?.totalOvers ?? currentMatchData?.overs} overs
              </p>
              <p className="text-xs text-base-content/60 mt-0.5">
                {newBattingTeam?.name} needs {inning1.runs + 1} runs to win
              </p>
            </div>
          )}
